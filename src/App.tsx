...
const controlIcons: Record<ControlType, typeof ShieldCheck> = { Patch: Wrench, Firewall: Network, SIEM: Siren, EDR: ShieldCheck, IAM: LockKeyhole, Configuration: SlidersHorizontal, "Risk Acceptance": FileCheck2 };
  ...
  return (
    <main className="shell">
      <header className="topbar">
        <div><div className="eyMark">EY CRVM Extension</div><h1>Attack Path Control Center</h1></div>
        ...
        <aside className="pathRail" aria-label="Attack paths">
          ...
          {attackPaths.map((path) => (
            <button className={`pathButton ${path.id === activePathId ? "active" : ""}`} key={path.id} onClick={() => setActivePathId(path.id)} type="button">
              <span><strong>{path.name}</strong><small>{path.entry} <ChevronRight size={12} /> {path.target}</small></span><b>{path.blastRadius}</b>
            </button>
          ))}
        </aside>
          ...
          <div className="panelGrid">
            <section className="findingsPanel" aria-label="Findings">
              ...
              {activePath.findings.map((finding) => {
                const risk = calculateResidualRisk(finding, selectedControlsForFinding(finding, allSelectedControls));
                return (
                  <article className="findingCard" key={finding.id}>
                    <div className="findingHeader"><div><div className="findingMeta"><span>{finding.cve}</span><span>{finding.source}</span></div><h3>{finding.title}</h3></div><span className={`severity ${severityClass[finding.rawSeverity]}`}>{finding.rawSeverity}</span></div>
                    <div className="scoreRow"><ScoreBlock label="Inherent" score={risk.inherentScore} severity={risk.rawSeverity} /><ScoreBlock label="Residual" score={risk.residualScore} severity={risk.residualSeverity} /></div>
                    <div className="signalGrid"><Signal active={finding.internetExposed} label="Internet exposed" /><Signal active={finding.exploitAvailable} label="Exploit known" /><Signal active={finding.lateralMovement} label="Lateral path" /><Signal active={finding.privilegedAsset} label="Privileged" /></div>
