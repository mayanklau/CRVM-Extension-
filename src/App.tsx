...
const approvalClass: Record<ApprovalStatus, string> = {
  Approved: "statusGood",
  "Pending Approval": "statusWarn",
  Draft: "statusNeutral",
  Rejected: "statusBad"
};

const validationClass: Record<ValidationStatus, string> = {
  Validated: "statusGood",
  "Drift Detected": "statusBad",
  "Pending Evidence": "statusWarn"
};

const connectorClass: Record<ConnectorStatus, string> = {
  Connected: "statusGood",
  "Needs Attention": "statusWarn",
  "Not Configured": "statusNeutral"
};

type View = "Command" | "Workbench" | "Governance" | "Integrations" | "Reports";
const views: View[] = ["Command", "Workbench", "Governance", "Integrations", "Reports"];
...
export function App() {
  ...
  const [selectedControlIds, setSelectedControlIds] = useState<Set<string>>(
    new Set(["ctrl-vpatch-CVE-2024-3094", "ctrl-fw-payments-edge", "ctrl-siem-t1190", "ctrl-iam-claims-admin"])
  );
  const activePath = attackPaths.find((path) => path.id === activePathId) ?? attackPaths[0];
  const visiblePaths = attackPaths.filter((path) =>
    `${path.name} ${path.entry} ${path.target}`.toLowerCase().includes(search.toLowerCase())
  );
  const allSelectedControls = useMemo(
    () => attackPaths.flatMap((path) => path.recommendations).filter((control) => selectedControlIds.has(control.id)),
    [selectedControlIds]
  );
    ...
    <main className="shell">
      <header className="topbar">
        ...
        <div className="topControls">
          <select aria-label="Active role" value={role} onChange={(event) => setRole(event.target.value)}>
            <option>CISO</option>
            <option>Vulnerability Manager</option>
      ...
      <nav className="productNav" aria-label="Product views">
        {views.map((view) => (
      ...
      <section className="commandStrip" aria-label="Operating context">
        <div><span>Operating as</span><strong>{role}</strong></div>
        <label>
          <span>Find attack path</span>
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search path, asset, service" />
        </label>
        <button type="button">Export brief</button>
        <button type="button">Create change</button>
      </section>
          ...
          <section className="pathPanel">
            ...
            <div className="panelGrid">
              ...
              <section className="controlsPanel" aria-label="Recommended controls">
                ...
                {activePath.recommendations.map((control) => (
                  <ControlCard control={control} key={control.id} selected={selectedControlIds.has(control.id)} onToggle={() => toggleControl(control.id)} />
                ))}
              </section>
            </div>
          </section>
      ...
      <section className="operationsGrid" aria-label="Enterprise control plane">
        {(activeView === "Command" || activeView === "Integrations") ? <Connectors /> : null}
        {(activeView === "Command" || activeView === "Workbench") ? <VirtualPatching /> : null}
        {(activeView === "Command" || activeView === "Governance") ? <Approvals /> : null}
        {(activeView === "Command" || activeView === "Workbench") ? <Validation /> : null}
        {(activeView === "Command" || activeView === "Workbench") ? <Notifications /> : null}
        {(activeView === "Command" || activeView === "Governance") ? <Evidence /> : null}
        {(activeView === "Command" || activeView === "Governance") ? <SlaHeat /> : null}
        {activeView === "Workbench" ? <Tickets /> : null}
        {activeView === "Integrations" ? <Ingestion /> : null}
        {activeView === "Reports" ? <Reports /> : null}
        {(activeView === "Command" || activeView === "Reports") ? <Brief /> : null}
      </section>
    </main>
  );
}
...
function ControlCard({ control, selected, onToggle }: { control: ControlRecommendation; selected: boolean; onToggle: () => void }) {
  ...
  return (
    <article className={`controlCard ${selected ? "selected" : ""}`}>
      ...
      <div className="controlBadges">
        {control.approvalStatus ? <Badge className={approvalClass[control.approvalStatus]} label={control.approvalStatus} /> : null}
        {control.validationStatus ? <Badge className={validationClass[control.validationStatus]} label={control.validationStatus} /> : null}
        {control.iacPullRequest ? <Badge className="statusInfo" label={`IaC PR ${control.iacPullRequest}`} /> : null}
      </div>
...
function Tickets() {
  return <section className="opsPanel widePanel"><div className="sectionTitle"><FileCheck2 size={18} /> Remediation work queue</div><div className="tablePanel">{remediationTickets.map((ticket) => <article className="tableRow" key={ticket.id}><strong>{ticket.id}</strong><span>{ticket.title}</span><Badge className={severityClass[ticket.priority]} label={ticket.priority} /><Badge className={ticket.status === "Blocked" ? "statusBad" : ticket.status === "Ready to Close" ? "statusGood" : "statusWarn"} label={ticket.status} /><small>{ticket.owner} via {ticket.system} by {ticket.due}</small></article>)}</div></section>;
}

function Ingestion() {
  return <section className="opsPanel widePanel"><div className="sectionTitle"><Filter size={18} /> Ingestion pipeline</div><div className="connectorGrid">{ingestionRuns.map((run) => <article className="connectorCard" key={run.id}><div><strong>{run.source}</strong><span>{run.records} records received</span></div><Badge className={run.status === "Healthy" ? "statusGood" : "statusWarn"} label={run.status} /><small>{run.normalized} normalized / {run.rejected} rejected</small><footer>Parser, mapper, deduper, and asset resolver completed</footer></article>)}</div></section>;
}

function Reports() {
  return <section className="opsPanel widePanel"><div className="sectionTitle"><FileCheck2 size={18} /> Report packs</div><div className="connectorGrid">{executiveReports.map((report) => <article className="connectorCard" key={report.id}><div><strong>{report.title}</strong><span>{report.audience}</span></div><Badge className={report.status === "Ready" ? "statusGood" : report.status === "Needs Evidence" ? "statusWarn" : "statusNeutral"} label={report.status} /><small>{report.sections.join(", ")}</small><footer>PDF, PPT, and audit appendix ready for generation</footer></article>)}</div></section>;
}

function Brief() {
  return <section className="opsPanel briefPanel"><div className="sectionTitle"><Activity size={18} /> LLM executive brief</div><h2>{executiveBrief.headline}</h2><p>{executiveBrief.narrative}</p><ul>{executiveBrief.talkingPoints.map((point) => <li key={point}>{point}</li>)}</ul></section>;
}
